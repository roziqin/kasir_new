...
$aa = "SELECT * from users where id='$aid'";
  ...
  $query=mysqli_query($con,"SELECT count(transaksi_id) as jumlah, sum(transaksi_total) as total, sum(transaksi_diskon) as diskon from transaksi where transaksi_tanggal='$tgl' and transaksi_user = '$iduser' group by transaksi_tanggal ");
      ...
      $query3=mysqli_query($con,"SELECT count(transaksi_id) as jumlah, sum(transaksi_total) as total, sum(transaksi_diskon) as diskon from transaksi where transaksi_tanggal='$tgl' and transaksi_user = '$iduser' and transaksi_type_bayar='goresto' group by transaksi_tanggal ");
      $datagoresto=mysqli_fetch_assoc($query3);
      $omsetgoresto = $datagoresto['total'];
...
<table  width="100%" border="0">
  ...
  <tr>
    <td>GoResto</td>
    <td>:</td>
    <td style="text-align: right;"><?php echo format_rupiah($omsetgoresto);?></td>
  </tr>
  <tr>
    <td>Total Omset</td>
    <td>:</td>
